...
def shift_pixel(x, sf, upper_left=True):
    """Shift pixel for super-resolution with different scale factors using RegularGridInterpolator
    Args:
        x: WxHxC or WxH
        sf: scale factor
        upper_left: shift direction
    ...
    if x.ndim == 2:
        ...
        for i in range(x.shape[-1]):
            interpolator = RegularGridInterpolator((yv, xv), x[:, :, i], method="linear", bounds_error=False,
                                                   fill_value=None)
            x_interpolated = interpolator(points)
            x[:, :, i] = x_interpolated.reshape(h, w)
    else:
...
def degradation_bsrgan(img, sf=4, lq_patchsize_h=72,lq_patchsize_w=72, isp_model=None, use_crop=True,d_mode='Noise'):
    """
...
def degradationBenchmark(img, sf=4, lq_patchsize_h=72,lq_patchsize_w=72, isp_model=None, use_crop=True,d_mode='bsrgan'):
    """
    This is the degradation model of BSRGAN from the paper
    "Designing a Practical Degradation Model for Deep Blind Image Super-Resolution"
    ----------
    ...
    if d_mode =='downsample':
        ...
        for i in shuffle_order:
            if i == 0:
                ...
                if random.random() < isp_prob and isp_model is not None:
                    with torch.no_grad():
                        img, hq = isp_model.forward(img.copy(), hq)
        img = cv2.resize(img, (int(1 / sf * h), int(1 / sf * w)), interpolation=random.choice([1, 2, 3]))
        img = np.clip(img, 0.0, 1.0)
        ...
        raise ValueError(f'd_mode {d_mode} not supported!')
...
def degradationBenchmarkPlus(img, sf=4, shuffle_prob=0.5, use_sharp=True, lq_patchsize_h=64,lq_patchsize_w=64, isp_model=None, use_crop=True,d_mode='bsrgan'):
    """
    This is the degradation model of BSRGAN from the paper
    "Designing a Practical Degradation Model for Deep Blind Image Super-Resolution"
    ----------
    ...
    hq: corresponding high-quality patch, size: (lq_patchsizexsf)X(lq_patchsizexsf)XC, range: [0, 1]
    """
    if d_mode == 'bsrganPlus':
        return degradation_bsrgan_plus(img,sf,0.5,True,lq_patchsize_h,lq_patchsize_w,isp_model,use_crop)
    ...
    if d_mode =='downsample':
        ...
        for i in shuffle_order:
            if i == 0:
                ...
                img = cv2.resize(img, (int(1 / sf * w), int(1 / sf * h)), interpolation=random.choice([1, 2, 3]))
                img = np.clip(img, 0.0, 1.0)
