...
from pipeline_stable_diffusion_img2img_condition import StableDiffusionImg2ImgPipeline_cond
import argparse
...
def main(args,gt_h,gt_w):
    ...
    dps_loss = DWTHFLoss(1, 1, 0.0, 100)
    for idx, path in enumerate(paths):
        ...
        img_ILR = img_tensor.resize((gt_h,gt_w), Image.Resampling.LANCZOS)

        with torch.enable_grad():
            if args.mode != 'DPS':
                output = sr_model(text_prompt,img_ILR,num_inference_steps=args.num_sampling_steps,eta=1,strength=args.strength).images[0]
            else:
                output = sr_model(text_prompt, img_ILR, num_inference_steps=args.num_sampling_steps, eta=1,image_tensor=img_tensor,strength=args.strength,dps_loss=dps_loss).images[0]
